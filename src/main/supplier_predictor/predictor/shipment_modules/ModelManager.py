...
class ModelManager:
    """
    Manages the training, evaluation, and prediction logic for the ML models.

    This class supports two distinct prediction paradigms: ensemble and
    hierarchical. The hierarchical approach is the primary, default model due
    to its superior performance in cross-validation, though the ensemble
    method is retained for benchmarking.

    Model Modes:
      - Ensemble: Trains multiple independent XGBoost classifiers on the full
        supplier-account label space and aggregates their votes. It is robust
        but can struggle with a large number of classes.
      - Hierarchical (Default): A two-stage model that first predicts the
        supplier (e.g., "DHL" vs. "FedEx") and then uses a supplier-specific
        sub-model to predict the account (e.g., "DHL account_A" vs. "DHL account_B").
        This approach is more precise as each model solves a narrower problem.
    """
    ...
    def run_training(self, csv_path, encoder_path, model_save_path):
        ...
        for fold, (train_idx, val_idx) in enumerate(cv.split(X, stratify_labels)):
            ...
            if self.mode == 'hierarchical':
                ...
                self._train_and_eval_ensemble(X_train, y_train, X_val, y_val, fold)
    ...
    def run_evaluation(self, csv_path, encoder_path, model_path):
        ...
        if model.mode == 'ensemble':
            ...
            model._print_classification_report(y, y_pred)
            model._plot_confusion_matrix(y, y_pred)
            model._plot_feature_importance()
        elif model.mode == 'hierarchical':
            ...
            #print(classification_report(y_true_sup, y_pred_sup))

            #print("\nAccount Classification Report:")
            #print(classification_report(y_true_acc, y_pred_acc))

            #model._plot_confusion_matrix(y_true_sup, y_pred_sup, title="Supplier Confusion Matrix")
            #model._plot_confusion_matrix(y_true_acc, y_pred_acc, title="Account Confusion Matrix")
    ...
    def _train_and_eval_ensemble(self, X_train, y_train, X_val, y_val, fold):
        X_res, y_res = SMOTE(random_state=self.random_state).fit_resample(X_train, y_train)
        counts = Counter(y_res)
        sample_weight = np.array([max(counts.values()) / counts[label] for label in y_res])

        model = XGBClassifier(
            objective='multi:softmax', eval_metric='mlogloss',
            learning_rate=0.1, max_depth=6, subsample=0.8, colsample_bytree=0.8,
            random_state=self.random_state, n_estimators=1000
        )
        model.fit(X_res, y_res, sample_weight=sample_weight, eval_set=[(X_val, y_val)], verbose=False)

        self.models.append(model)
        y_pred = model.predict(X_val)
        self._store_metrics(y_val, y_pred)
        self._store_feature_importances(model)
        print(f"Fold {fold+1} - "
              f"Accuracy: {accuracy_score(y_val, y_pred):.4f} | "
              f"F1: {f1_score(y_val, y_pred, average='macro'):.4f} | "
              f"Recall: {recall_score(y_val, y_pred, average='macro'):.4f}")

    def _train_and_eval_hierarchical(self, X_train, y_train, X_val, y_val, fold):
    ...
    def predict(self, X, use_ensemble=True):
        if self.mode == 'ensemble':
            if use_ensemble:
                predictions = np.mean([m.predict(X) for m in self.models], axis=0)
    ...
    def _store_metrics(self, y_true, y_pred):
        self.cv_metrics['accuracy'].append(accuracy_score(y_true, y_pred))
        self.cv_metrics['recall'].append(recall_score(y_true, y_pred, average='macro'))
        self.cv_metrics['f1'].append(f1_score(y_true, y_pred, average='macro'))
        self.cv_metrics['all_true'].extend(y_true)
        self.cv_metrics['all_pred'].extend(y_pred)

    def _store_feature_importances(self, model):
    ...
    def _calculate_final_metrics(self):
        if self.feature_importances_ is not None:
            self.feature_importances_ /= self.n_splits
        self.cv_metrics['mean_accuracy'] = np.mean(self.cv_metrics['accuracy'])
        self.cv_metrics['mean_recall'] = np.mean(self.cv_metrics['recall'])
        self.cv_metrics['mean_f1'] = np.mean(self.cv_metrics['f1'])
    ...
    def load_model(cls, path):
        ...
        if manager.mode == 'ensemble':
            manager.models = data['models']
        elif manager.mode == 'hierarchical':
