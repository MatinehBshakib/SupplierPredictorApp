...
In your `settings.py` file, add your Heroku app's URL to `ALLOWED_HOSTS`:

```python
ALLOWED_HOSTS = ['your-unique-app-name.herokuapp.com', '127.0.0.1']
```

Commit your changes and push to Heroku to deploy:
...
Run database migrations on the Heroku server:
...
You can view your application's logs at any time with the command:
